...
class ILD:
    ...
    def calculate_ild(self, recommendation_articles, representation='tfidf'): 
        ...
        if isinstance(recommendation_articles, torch.Tensor):
            vecs = recommendation_articles.detach().cpu().numpy()
        else:
            vecs = np.array(recommendation_articles)
        ...
        #else:
        #    X = np.array(vecs)
        #print(recommendation_articles[:10])

        # Compute cosine-similarity matrix (works with sparse matrices)
        sim_matrix = cosine_similarity(vecs)  # shape (n, n)
