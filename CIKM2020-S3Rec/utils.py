...
def gini_coefficient(recommended_items):
    """
    Compute Gini coefficient for item distribution in recommendation results.

    Args:
        recommended_items (List[List[int]]): List of top-K recommendations per user.
    
    Returns:
        float: Gini coefficient (0 = perfect equality, 1 = max inequality).
    """
    from collections import Counter
    all_recommended = [item for user_items in recommended_items for item in user_items]
    item_counts = Counter(all_recommended)
    values = np.array(sorted(item_counts.values()))
